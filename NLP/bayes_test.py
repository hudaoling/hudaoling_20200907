...
def train_or_load_classifier():
    ...
    return NaiveBayesClassifier(model) #classifier分类器


def predict(classifier, text):
    print("《%16s》\t属于分类\t【%s】" % (text, classifier.classify(text)))   #classifier.classify分类器
...
if __name__ == '__main__':
    ...
    predict(classifier, "NBA复赛")
    predict(classifier, "春秋航空特价旅游")
